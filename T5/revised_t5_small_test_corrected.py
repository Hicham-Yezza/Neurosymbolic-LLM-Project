...
def preprocess_data(dataset, max_length=512, batch_size=4):
    ...
    # Shifting the target sequences to the right to generate decoder_input_ids
    dataset = dataset.map(
        lambda examples: {'decoder_input_ids': tokenizer.pad(tokenizer(examples['summary'], return_tensors='pt', truncation=True, padding='max_length')['input_ids'], return_tensors='pt', padding=True, max_length=max_length).squeeze(0)},
        batched=True
    )
